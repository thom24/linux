...
static struct upboard_led_profile upboard_led_profile[] = {
	{ "upboard:yellow:" LED_FUNCTION_STATUS, LEDS_DEFSTATE_KEEP, 0 },
	{ "upboard:green:" LED_FUNCTION_STATUS, LEDS_DEFSTATE_KEEP, 1 },
	{ "upboard:red:" LED_FUNCTION_STATUS, LEDS_DEFSTATE_KEEP, 2 },
};

static struct upboard_led_profile upboard_led_profile_2[] = {
	{ "upboard:blue:" LED_FUNCTION_STATUS, LEDS_DEFSTATE_KEEP, 0 },
	{ "upboard:yellow:" LED_FUNCTION_STATUS, LEDS_DEFSTATE_KEEP, 1 },
	{ "upboard:green:" LED_FUNCTION_STATUS, LEDS_DEFSTATE_KEEP, 2 },
	{ "upboard:red:" LED_FUNCTION_STATUS, LEDS_DEFSTATE_KEEP, 3 },
};
...
static int upboard_led_probe(struct platform_device *pdev)
{
	...
	case UPBOARD_UP_FPGA:
		led_profile = upboard_led_profile;
		led_instances = ARRAY_SIZE(upboard_led_profile);
		break;
	case UPBOARD_UP2_FPGA:
		led_profile = upboard_led_profile_2;
		led_instances = ARRAY_SIZE(upboard_led_profile_2);
		break;
	default:
	...
	for (i = 0; i < led_instances; i++) {
		...
		switch (led_profile[i].state) {
		case LEDS_DEFSTATE_ON:
			led->cdev.brightness = led->cdev.max_brightness;
			upboard_led_brightness_set(&led->cdev, led->cdev.max_brightness);
			break;
		case LEDS_DEFSTATE_KEEP:
			led->cdev.brightness = upboard_led_brightness_get(&led->cdev);
			break;
		default:
			led->cdev.brightness = LED_OFF;
			upboard_led_brightness_set(&led->cdev, LED_OFF);
		}
