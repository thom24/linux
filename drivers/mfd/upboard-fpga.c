...
static int upboard_fpga_get_firmware_info(struct upboard_fpga *fpga)
{
	...
	if (manufacturer_id != UPBOARD_AAEON_MANUFACTURER_ID)
		return dev_err_probe(fpga->dev, -ENODEV,
				     "driver not compatible with custom FPGA FW from manufacturer id 0x%02x.",
				     manufacturer_id);

	ret = regmap_read(fpga->regmap, UPBOARD_REG_FIRMWARE_ID, &firmware_id);
	if (ret)
		return ret;

	fpga->info.build = FIELD_GET(UPBOARD_FW_ID_BUILD_MASK, firmware_id);
	fpga->info.major = FIELD_GET(UPBOARD_FW_ID_MAJOR_MASK, firmware_id);
	fpga->info.minor = FIELD_GET(UPBOARD_FW_ID_MINOR_MASK, firmware_id);
	fpga->info.patch = FIELD_GET(UPBOARD_FW_ID_PATCH_MASK, firmware_id);

	if (fpga->info.major != UPBOARD_FW_ID_MAJOR_SUPPORTED)
		return dev_err_probe(fpga->dev, -ENODEV,
				     "unsupported FPGA FW v%u.%u.%u build 0x%02x",
				     fpga->info.major, fpga->info.minor, fpga->info.patch,
				     fpga->info.build);
					 ...
					 char *buf)
{
	...
	return sysfs_emit(buf, "FPGA FW v%u.%u.%u build 0x%02x\n", fpga->info.major,
			  fpga->info.minor, fpga->info.patch, fpga->info.build);
}
...
static int upboard_fpga_probe(struct platform_device *pdev)
{
	...
	fpga->fpga_data = (const struct upboard_fpga_data *)device_get_match_data(dev);
	...
	ret = upboard_fpga_get_firmware_info(fpga);
	if (ret)
		return ret;

	return devm_mfd_add_devices(dev, PLATFORM_DEVID_AUTO, upboard_up_mfd_cells,
