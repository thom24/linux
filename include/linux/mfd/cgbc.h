...
struct cgbc_platform_data {
	...
	int	(*command)(struct cgbc_device_data *cgbc,
			   u8 *cmd, u8 cmd_size,
			   u8 *data, u8 data_size,
			   u8 *status);
	int	(*register_cells)(struct cgbc_device_data *cgbc);
	int	(*map)(struct cgbc_device_data *cgbc);
	int	(*init)(struct cgbc_device_data *cgbc);
	void	(*close)(struct cgbc_device_data *cgbc);
};
